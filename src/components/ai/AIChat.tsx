...
export function AIChat() {
  ...
  return (
    <div className="flex flex-col h-[70dvh] min-h-[420px] max-h-[720px] md:h-[calc(100vh-200px)] md:max-h-[800px] bg-white rounded-2xl shadow-card overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-[#FF6A3D] to-[#F4511E]">
        ...
        <div className="space-y-4">
          {messages.map((message) => (
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                message.role === 'user'
                  ? 'bg-[#FF6A3D]'
                  : 'bg-gradient-to-br from-[#FF6A3D] to-[#F4511E]'
              }`}>
                {message.role === 'user' ? (
              ...
              <div className={`max-w-[80%] p-3 rounded-2xl ${
                message.role === 'user'
                  ? 'bg-[#FF6A3D] text-white rounded-tr-none'
          ...
          {isLoading && (
            <div className="flex gap-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#FF6A3D] to-[#F4511E] flex items-center justify-center">
                <Bot className="w-4 h-4 text-white" />
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {suggestions.map((suggestion, index) => (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-[#FFF1EA] text-[#FF6A3D] rounded-full text-sm hover:bg-[#FF6A3D] hover:text-white transition-colors"
                >
                  {suggestion.icon}
                  {suggestion.text}
                </button>
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-100">
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="h-12 w-12 rounded-full bg-[#FF6A3D] hover:bg-[#F4511E] p-0"
          >
